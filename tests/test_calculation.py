...
def test_calculation_negative_cases():
    # --- Division by zero ---
    with pytest.raises(OperationError) as exc_info:
        Calculation(operation="Dision", operand1=Decimal(10), operand2=Decimal(0))
    assert "Unknown operation: Dision" in str(exc_info.value)

def test_calculation_string():
    calc = Calculation(operation="Division", operand1=Decimal("1"), operand2=Decimal("3"))

    assert calc.__str__() == "Division(1, 3) = 0.3333333333333333333333333333"